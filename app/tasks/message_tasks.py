...
def process_incoming_message(raw: dict):
    ...
    # Send initial greeting to acknowledge the user
    asyncio.run(_send_greeting(msg.phone, whatsapp))

    # if user is confirming, deliver any pending contacts
    if msg.text.strip().lower() in ("yes", "y"):
        contacts = asyncio.run(_deliver_contacts(msg.phone))
        if contacts:
            asyncio.run(
    ...
    asyncio.run(_store_user_and_match(context, msg.phone))
